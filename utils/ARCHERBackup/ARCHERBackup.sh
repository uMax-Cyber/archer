...
usage() {
    ...
    echo -e "\t-B -b --backuppath\t\tSpecify the backup path.\n\t\tIf not set will use backupPath from archer settings plus archer_backup_DATE."
...
backupDB() {
    ...
    wget --no-check-certificate --post-data="nojson=1" -O $backupDir/mysql/archer.sql "http://$ipaddress/$webroot/management/export.php?type=sql" 2>>$backupDir/logs/error.log 1>>$backupDir/logs/progress.log 2>&1
    stat=$?
    if [[ ! $stat -eq 0 ]]; then
        echo "Failed"
...
backupSnapins() {
    [[ -z $snapinLocation ]] && snapinLocation='/opt/archer/snapins'
    [[ ! -d $snapinLocation ]] && handleError "Snapins location:$snapinLocation does not exist on this server. Please add snapinLocation='/path/to/snapins' to .archersettings." 16
