#!/bin/bash
#
#  ARCHER is a computer imaging solution.
...
workingdir=$(pwd)
if [[ ! $EUID -eq 0 ]]; then
    echo "ARCHER Installation must be run as root user"
    exit 1
fi
...
help() {
    ...
    echo -e "\t\t[-W <webroot/to/archer/after/docroot/>] [-B </backup/path/>]"
    ...
    echo -e "\t               \t\t\t\tdefaults to /opt/archer/snapins/ssl"
    echo -e "\t-D    --docroot\t\t\tSpecify the Apache Docroot for archer"
    echo -e "\t               \t\t\t\tdefaults to OS DocumentRoot"
    echo -e "\t-W    --webroot\t\t\tSpecify the web root url want archer to use"
    echo -e "\t            \t\t\t\t(E.G. http://127.0.0.1/archer,"
    echo -e "\t            \t\t\t\t      http://127.0.0.1/)"
    echo -e "\t            \t\t\t\tDefaults to /archer/"
    echo -e "\t-B    --backuppath\t\tSpecify the backup path"
    echo -e "\t      --uninstall\t\tUninstall ARCHER"
    ...
    case $o in
        -)
            case $OPTARG in
                ...
                webroot)
                    if [[ $OPTARG != *('/')* ]]; then
                        echo -e "-$OPTARG needs a url path for access either / or /archer for example.\n\n\t\tfor example if you access archer using http://127.0.0.1/ without any trail\n\t\tset the path to /"
                        help
                        exit 2
                    fi
                    swebroot="${OPTARG}"
...
[[ -z $version ]] && version="$(awk -F\' /"define\('ARCHER_VERSION'[,](.*)"/'{print $4}' ../packages/web/lib/archer/system.class.php | tr -d '[[:space:]]')"
[[ -z $OS ]] && OS=$(uname -s)
...
command -v lsb_release >$workingdir/error_logs/archer_error_${version}.log 2>&1
exitcode=$?
if [[ ! $exitcode -eq 0 ]]; then
    case $linuxReleaseName in
        *[Bb][Ii][Aa][Nn]*|*[Uu][Bb][Uu][Nn][Tt][Uu]*|*[Mm][Ii][Nn][Tt]*)
            apt-get -yq install lsb-release >>$workingdir/error_logs/archer_error_${version}.log 2>&1
            ;;
        *[Cc][Ee][Nn][Tt][Oo][Ss]*|*[Rr][Ee][Dd]*[Hh][Aa][Tt]*|*[Ff][Ee][Dd][Oo][Rr][Aa]*)
            command -v dnf >>$workingdir/error_logs/archer_error_${version}.log 2>&1
            exitcode=$?
            case $exitcode in
                0)
                    dnf -y install redhat-lsb-core >>$workingdir/error_logs/archer_error_${version}.log 2>&1
                    ;;
                *)
                    yum -y install redhat-lsb-core >>$workingdir/error_logs/archer_error_${version}.log 2>&1
                    ;;
            esac
            ;;
        *[Aa][Rr][Cc][Hh]*)
            pacman -Sy --noconfirm lsb-release >>$workingdir/error_logs/archer_error_${version}.log 2>&1
            ;;
    esac
fi
...
[[ -z $mysqldbname ]] && mysqldbname="archer"
[[ -z $archerpriorconfig ]] && archerpriorconfig="$archerprogramdir/.archersettings"
#clearScreen
if [[ -z $* || $* != +(-h|-?|--help|--uninstall) ]]; then
    echo > "$workingdir/error_logs/archerinstall.log"
    exec &> >(tee -a "$workingdir/error_logs/archerinstall.log")
fi
displayBanner
...
case $doupdate in
    1)
        if [[ -f $archerpriorconfig ]]; then
            echo -e "\n * Found ARCHER Settings from previous install at: $archerprogramdir/.archersettings\n"
        ...
        ;;
    *)
        echo -e "\n * ARCHER Installer will NOT attempt to upgrade from\n    previous version of ARCHER."
        ;;
esac
...
[[ -f $archerpriorconfig ]] && grep -l webroot $archerpriorconfig >>$workingdir/error_logs/archer_error_${version}.log 2>&1
case $? in
    0)
        if [[ -n $webroot ]]; then
            webroot=${webroot#'/'}
...
[[ ! $doupdate -eq 1 || ! $archerupdateloaded -eq 1 ]] && . ../lib/common/input.sh
# ask user input for newly added options like hostname etc.
. ../lib/common/newinput.sh
echo
echo "   ######################################################################"
echo "   #     ARCHER now has everything it needs for this setup, but please     #"
...
echo "   ######################################################################"
echo "   #             https://wiki.archerproject.org/wiki/index.php             #"
echo "   ######################################################################"
echo
echo " * Here are the settings ARCHER will use:"
...
case $installtype in
    N)
        ...
        case $bldhcp in
            1)
                echo " * Using ARCHER DHCP: Yes"
                echo " * DHCP router Address: $plainrouter"
                ;;
            *)
                echo " * Using ARCHER DHCP: No"
                echo " * DHCP will NOT be setup but you must setup your"
                echo " | current DHCP server to use ARCHER for PXE services."
                echo
                ...
                echo
                echo " * Option 066/next-server is the IP of the ARCHER Server: (e.g. $ipaddress)"
...
while [[ -z $blGo ]]; do
    ...
    case $blGo in
        [Yy]|[Yy][Ee][Ss])
            ...
            case $installtype in
                [Ss])
                    ...
                    if [[ $bluseralreadyexists == 1 ]]; then
                        ...
                        [[ -n $snmysqlhost ]] && archerserver=$snmysqlhost || archerserver="archer-server"
                        echo
                        echo " * Setup complete"
                        echo
                        echo
                        echo " * You still need to setup this node in the archer management "
                        echo " | portal. You will need the username and password listed"
                        echo " | below."
                        echo
                        echo " * Management Server URL:"
                        echo "   ${httpproto}://${archerserver}${webroot}"
                        echo
                ...
                [Nn])
                    ...
                    echo " * Setup complete"
                    echo
                    echo "   You can now login to the ARCHER Management Portal using"
            ...
            ;;
        [Nn]|[Nn][Oo])
            echo " * ARCHER installer exited by user request"
            exit 0
            ;;
        *)
            echo
