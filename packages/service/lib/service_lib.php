...
$service_logpath = sprintf(
    '/%s/%s',
    trim(ARCHERCore::getSetting('SERVICE_LOG_PATH'), '/'),
    ARCHERCore::getSetting('SERVICEMASTERLOGFILENAME')
);
if (!is_file($service_logpath)) {
    $service_logpath = '/opt/archer/log/servicemaster.log';
}
$service_sleep_time = (int)ARCHERCore::getSetting('SERVICESLEEPTIME');
...
function Service_Log_message($logpath, $name, $msg)
{
    ...
    $msg = sprintf(
        "[%s] %s %s\n",
        ARCHERCore::formatTime('now', 'm-d-y g:i:s a'),
        $name,
        $msg
    );
    fwrite($logfile, $msg);
