 ...
 * @link     https://archer.umax.uz
 */
class SnapinManagementPage extends ARCHERPage
{
    /**
    ...
    public function __construct($name = '')
    {
        ...
        /**
         * Pull in the ARCHER Page class items.
         */
        ...
        if ($id) {
             ...
             */
            $this->subMenu = array(
                "$this->linkformat#snap-gen" => self::$archerlang['General'],
             ...
             */
            $this->notes = array(
                self::$archerlang['Snapin'] => $this->obj->get('name'),
                self::$archerlang['File'] => $this->obj->get('file'),
    ...
    private function _maker()
    {
        $args = array(
            'MSI' => array(
                'msiexec.exe',
                '/i &quot;[ARCHER_SNAPIN_PATH]\\MyMSI.msi&quot;'
            ),
            'MSI + MST' => array(
                'msiexec.exe',
                '/i &quot;[ARCHER_SNAPIN_PATH]\\MyMST.mst&quot;'
            ),
            'Batch Script' => array(
                'cmd.exe',
                '/c &quot;[ARCHER_SNAPIN_PATH]\\MyScript.bat&quot;'
            ),
            'Bash Script' => array(
                '/bin/bash',
                '&quot;[ARCHER_SNAPIN_PATH]/MyScript.sh&quot;'
            ),
            'VB Script' => array(
                'cscript.exe',
                '&quot;[ARCHER_SNAPIN_PATH]\\MyScript.vbs&quot;'
            ),
            'PowerShell Script' => array(
                'powershell.exe',
                '-ExecutionPolicy Bypass -File &quot;'
                .'[ARCHER_SNAPIN_PATH]\\MyScript.ps1&quot;'
            ),
    ...
    public function addPost()
    {
        ...
        try {
            ...
            if ($uploadfile && file_exists($src)) {
                ...
                if (!self::$ARCHERFTP->chdir($StorageNode->get('snapinpath'))) {
                    if (!self::$ARCHERFTP->mkdir($StorageNode->get('snapinpath'))) {
                        throw new Exception(
                            _('Failed to add snapin')
                        );
                    }
                }
                self::$ARCHERFTP->delete($dest);
                if (!self::$ARCHERFTP->put($dest, $src)) {
                    throw new Exception(
