 ...
 * @link     https://archer.umax.uz
 */
class StorageManagementPage extends ARCHERPage
{
    /**
            ...
            if ($id) {
                ...
                $this->notes = array(
                    sprintf(
                        ...
                        self::$archerlang['Node']
                    ) => $this->obj->get('name'),
                    self::$archerlang['ImagePath'] => $this->obj->get('path'),
                    self::$archerlang['FTPPath'] => $this->obj->get('ftppath'),
                );
            }
            break;
    ...
    public function index()
    {
        ...
        $this->title = self::$archerlang['AllSN'];
        ...
        $this->headerData = array(
            ...
            self::$archerlang['SN'],
            self::$archerlang['SG'],
            self::$archerlang['Enabled'],
            self::$archerlang['MasterNode'],
            _('Max Clients'),
        );
    ...
    {
        $this->title = self::$archerlang['AddSN'];
        ...
        $webroot = filter_input(INPUT_POST, 'webroot') ?: '/archer';
        ...
        $snapinpath = filter_input(INPUT_POST, 'snapinpath') ?: '/opt/archer/snapins/';
        $sslpath = filter_input(INPUT_POST, 'sslpath') ?: '/opt/archer/snapins/ssl/';
        ...
        $fields = array(
            ...
            . self::$archerlang['CautionPhrase']
            . '" data-toggle="tooltip" data-placement="right"></i>'
            . '</div>',
            '<label for="bandwidth">'
            . self::$archerlang['BandwidthReplication']
            . ' (Kbps)'
            . '</label>' => '<div class="input-group">'
            . '<i class="input-group-addon icon fa fa-question hand" title="'
            . self::$archerlang['BandwidthRepHelp']
            ...
            . '</div>',
            '<label for="isen">'
            . self::$archerlang['IsEnabled']
            . '</label>' => '<input type="checkbox" name="isEnabled" id="isen" '
            . 'checked/>',
            '<label for="isgren">'
            . self::$archerlang['IsGraphEnabled']
            . '<br/>'
            . '('
            . self::$archerlang['OnDash']
            . ')'
            . '</label>' => '<input type="checkbox" name="isGraphEnabled" '
            . 'id="isgren" checked/>',
            '<label for="user">'
            . self::$archerlang['ManUser']
            . '</label>' => '<div class="input-group">'
    ...
    public function addStorageNodePost()
    {
        ...
        try {
            if (empty($name)) {
                throw new Exception(self::$archerlang['StorageNameRequired']);
            }
            if (self::getClass('StorageNodeManager')->exists($name)) {
                throw new Exception(self::$archerlang['StorageNameExists']);
            }
            if (empty($ip)) {
                throw new Exception(self::$archerlang['StorageIPRequired']);
            }
            if (empty($maxClients)) {
                throw new Exception(self::$archerlang['StorageClientsRequired']);
            }
            if (empty($interface)) {
                throw new Exception(self::$archerlang['StorageIntRequired']);
            }
            if (empty($user)) {
                throw new Exception(self::$archerlang['StorageUserRequired']);
            }
            if (empty($pass)) {
                throw new Exception(self::$archerlang['StoragePassRequired']);
            }
    ...
    public function editStorageNodePost()
    {
        ...
        try {
            ...
            if ($maxClients < 0) {
                throw new Exception(self::$archerlang['StorageClientRequired']);
            }
            if (!$interface) {
                throw new Exception(self::$archerlang['StorageIntRequired']);
            }
    ...
    {
        if (self::getSetting('ARCHER_REAUTH_ON_DELETE')) {
            $user = filter_input(INPUT_POST, 'archerguiuser');
            $pass = filter_input(INPUT_POST, 'archerguipass');
            ...
            if ($validate) {
                echo json_encode(
                    array(
                        'error' => self::$archerlang['InvalidLogin']
                    )
                );
                exit;
            }
        ...
        try {
            if (!$this->obj->destroy()) {
                throw new Exception(self::$archerlang['FailDelSN']);
            }
            $hook = 'STORAGE_NODE_DELETE_SUCCESS';
            $msg = sprintf(
                '%s: %s',
                self::$archerlang['SNDelSuccess'],
                $this->obj->get('name')
            );
            $url = sprintf(
                '?node=%s',
    ...
    public function storageGroup()
    {
        ...
        $this->title = self::$archerlang['AllSG'];
    ...
    {
        $this->title = self::$archerlang['AddSG'];
        unset($this->headerData);
        $this->attributes = array(
            array('class' => 'col-xs-4'),
            array('class' => 'col-xs-8 form-group'),
    ...
    public function addStorageGroupPost()
    {
        ...
        try {
            if (empty($name)) {
                throw new Exception(self::$archerlang['SGNameReq']);
            }
            if (self::getClass('StorageGroupManager')->exists($name)) {
                throw new Exception(self::$archerlang['SGExist']);
            }
            $StorageGroup = self::getClass('StorageGroup')
                ->set('name', $name)
                ->set('description', $desc);
            if (!$StorageGroup->save()) {
                throw new Exception(self::$archerlang['DBupfailed']);
            }
            $hook = 'STORAGE_GROUP_ADD_POST_SUCCESS';
            $msg = json_encode(
                array(
                    'msg' => self::$archerlang['SGCreated'],
    ...
    public function editStorageGroupPost()
    {
        ...
        try {
            ...
            if (!$name) {
                throw new Exception(self::$archerlang['SGName']);
            }
            if ($this->obj->get('name') != $name
                && $exists
            ) {
    ...
    public function deleteStorageGroupPost()
    {
        ...
        try {
            if (self::getClass('StorageGroupManager')->count() == 1) {
                throw new Exception(self::$archerlang['OneSG']);
            }
            if (!$this->obj->destroy()) {
                throw new Exception(self::$archerlang['FailDelSG']);
            }
            $hook = 'STORAGE_GROUP_DELETE_POST_SUCCESS';
            $msg = sprintf(
                '%s: %s',
                self::$archerlang['SGDelSuccess'],
                $this->obj->get('name')
            );
            $url = sprintf(
