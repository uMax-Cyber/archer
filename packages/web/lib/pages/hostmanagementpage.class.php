 ...
 * @link     https://archer.umax.uz
 */
class HostManagementPage extends ARCHERPage
{
    /**
    ...
    public function __construct($name = '')
    {
        ...
        if (self::$pendingHosts > 0) {
            $this->menu['pending'] = self::$archerlang['PendingHosts'];
        }
        global $id;
        if ($id) {
            ...
            $this->subMenu = self::fastmerge(
                ...
                array(
                    ...
                    sprintf(
                        $linkstr,
                        'printers'
                    ) => self::$archerlang['Printers'],
                    sprintf(
                        $linkstr,
                        'snapins'
                    ) => self::$archerlang['Snapins'],
                    sprintf(
                        $linkstr,
                        'service'
                    ) => sprintf(
                        '%s %s',
                        self::$archerlang['Service'],
                        self::$archerlang['Settings']
                    ),
                    sprintf(
                        $linkstr,
                        'powermanagement'
                    ) => self::$archerlang['PowerManagement'],
                    sprintf(
                        $linkstr,
                        'hardware-inventory'
                    ) => self::$archerlang['Inventory'],
                    sprintf(
                        $linkstr,
                        'virus-history'
                    ) => self::$archerlang['VirusHistory'],
                    sprintf(
                        $linkstr,
                        'login-history'
                    ) => self::$archerlang['LoginHistory'],
                    sprintf(
                        $linkstr,
                        'image-history'
                    ) => self::$archerlang['ImageHistory'],
                    sprintf(
                        $linkstr,
                        'snapin-history'
                    ) => self::$archerlang['SnapinHistory'],
                    $this->membership => self::$archerlang['Membership'],
                    $this->delformat => self::$archerlang['Delete'],
                )
            );
            $this->notes = array(
                self::$archerlang['Host'] => $this->obj->get('name'),
                self::$archerlang['MAC'] => $this->obj->get('mac'),
                self::$archerlang['Image'] => $this->obj->getImageName(),
                self::$archerlang['LastDeployed'] => $this->obj->get('deployed'),
            );
            $primaryGroup = @min($this->obj->get('groups'));
            $Group = new Group($primaryGroup);
            if ($Group->isValid()) {
                $this->notes[self::$archerlang['PrimaryGroup']] = $Group->get('name');
                unset($Group);
            }
        }
        ...
        self::$archerpingactive ? array_push($this->headerData, '') : null;
        array_push(
            $this->headerData,
            _('Host'),
            _('Imaged'),
    ...
    public function add()
    {
        ...
        if (!isset($_POST['enforcesel'])) {
            $_POST['enforcesel'] = self::getSetting('ARCHER_ENFORCE_HOST_CHANGES');
        }
        echo '<!-- Host General -->';
        $this->render(12);
        echo '</div>';
    ...
    public function hostPrinters()
    {
        ...
        echo '<label for="nolevel" data-toggle="tooltip" data-placement="left" '
            . 'title="'
            . _('This setting turns off all ARCHER Printer Management')
            . '. '
        ...
        echo '<label for="addlevel" data-toggle="tooltip" data-placement="left" '
            ...
            . _(
                ...
                . 'printers that are managed by ARCHER. '
        ...
        echo '<input type="radio" name="level" value="1" '
            ...
            . '/>';
        echo _('ARCHER Managed Printers');
        echo '</label>';
        echo '</div>';
        echo '<div class="radio">';
        echo '<label for="alllevel" data-toggle="tooltip" data-placement="left" '
            . 'title="'
            . _(
                'This setting will only allow ARCHER Assigned '
                . 'printers to be added to the host. Any '
                . 'printer that is not assigned will be '
                . 'removed including non-ARCHER managed printers.'
            )
            . '">';
    ...
    public function hostService()
    {
        ...
        $dcnote = sprintf(
            ...
            _('The old client is what was distributed with ARCHER 1.2.0 and earlier'),
        ...
        echo '<div class="panel-heading text-center">';
        echo '<h4 class="title">';
        echo _('Host ARCHER Client Module configuration');
        echo '</h4>';
        echo '</div>';
        echo '<div class="panel-body">';
        echo '<form class="form-horizontal" method="post" action="'
