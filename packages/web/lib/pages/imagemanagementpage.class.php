 ...
 * @link     https://archer.umax.uz
 */
class ImageManagementPage extends ARCHERPage
{
    /**
    ...
    public function __construct($name = '')
    {
        ...
        /**
         * Pull in the ARCHERPage class items.
         */
         ...
         */
        $SizeServer = self::getSetting('ARCHER_FTP_IMAGE_SIZE');
        /**
         * Get our nicer names.
         */
        global $id;
        ...
        if ($id) {
             ...
             */
            $this->subMenu = array(
                "$this->linkformat#image-gen" => self::$archerlang['General'],
             ...
             */
            $this->notes = array(
                self::$archerlang['Images'] => $this->obj->get('name'),
                self::$archerlang['LastCaptured'] => $this->obj->get('deployed'),
                self::$archerlang['DeployMethod'] => (
                    $this->obj->get('format') ?
                    _('Partimage') :
                    _('Partclone')
                ),
                self::$archerlang['ImageType'] => (
                    $this->obj->getImageType() ?
                    $this->obj->getImageType() :
                    self::$archerlang['NoAvail']
                ),
    ...
    public function add()
    {
        ...
        $compression = self::getSetting('ARCHER_PIGZ_COMP');
        if ($compress < 0 || $compress > 23) {
            $compression = $compress;
        }
        if (!isset($imagemanage)) {
            $imagemanage = self::getSetting('ARCHER_IMAGE_COMPRESSION_FORMAT_DEFAULT');
        }
        $format = sprintf(
            ...
            if ($file == 'postdownloadscripts'
                ...
                throw new Exception(
                    sprintf(
                        ...
                        _('this one is reserved for ARCHER')
                    )
                );
            }
    ...
    public function imageGeneral()
    {
        ...
        $fields = array(
            ...
            . '<a href=\'https://wiki.archerproject.org/wiki/index.php?title=Managing_ARCHER#Images\' '
    ...
    public function multicast()
    {
        ...
        $this->title = self::$archerlang['Multicast'];
        $this->attributes = array(
            array('class' => 'col-xs-4'),
            array('class' => 'col-xs-8 form-group'),
        );
    ...
    public function multicastPost()
    {
        try {
            ...
            if ($timeout > 0) {
                self::setSetting('ARCHER_UDPCAST_MAXWAIT', $timeout);
            }
            ...
            $countmctot = self::getSetting('ARCHER_MULTICAST_MAX_SESSIONS');
            ...
            $MulticastSession = self::getClass('MulticastSession')
                ->set('name', $name)
                ->set('port', self::getSetting('ARCHER_UDPCAST_STARTINGPORT'))
            ...
            self::setSetting('ARCHER_UDPCAST_STARTINGPORT', $randomnumber);
            self::setMessage(
                sprintf(
                    '%s<br/>%s %s %s',
