 ...
 * @link     https://archer.umax.uz/
 */
class Route extends ARCHERBase
{
    /**
    ...
    public function __construct()
    {
        ...
        if (!self::$_enabled) {
            header(
                sprintf(
                    'Location: %s://%s/archer/management/index.php',
                    self::$httpproto,
                    self::$httphost
                )
            );
    ...
    private static function _testToken()
    {
        $passtoken = base64_decode(
            filter_input(INPUT_SERVER, 'HTTP_ARCHER_API_TOKEN')
        );
    ...
    private static function _testAuth()
    {
        $usertoken = base64_decode(
            filter_input(INPUT_SERVER, 'HTTP_ARCHER_USER_TOKEN')
        );
        ...
        $auth = self::$ARCHERUser->passwordValidate(
