<?php
/**
 * Handles the archer linux services
 *
 * PHP version 5
 *
 * @category ARCHERService
 ...
 * @link     https://archer.umax.uz
 */
abstract class ARCHERService extends ARCHERBase
{
    /**
    ...
    /**
     * Initializes the ARCHERService class
     *
     * @return void
     */
    ...
    public function waitInterfaceReady()
    {
        ...
        if (!count(self::$ips) || !in_array(self::getSetting('ARCHER_WEB_HOST'), self::$ips)) {
            self::outall(
                sprintf(
                    '%s: %s',
    ...
    /**
     * Displays the banner for archer services
     *
     * @return void
     */
    public function getBanner()
    {
        ...
        echo "==================================\n";
        echo "============ Credits =============\n";
        echo "= https://archer.umax.uz/Credits =\n";
        echo "==================================\n";
    ...
    protected function replicateItems(
        ...
        if ($groupOrNodeCount < $counttest) {
            ...
            foreach ($StorageNodes as $i => &$StorageNode) {
                ...
                $sizeurl = sprintf('%s://%s/archer/status/getsize.php', self::$httpproto, $testip);
                $hashurl = sprintf('%s://%s/archer/status/gethash.php', self::$httpproto, $testip);
                self::$ARCHERFTP
                ...
                $username = self::$ARCHERFTP->get('username');
                $password = self::$ARCHERFTP->get('password');
                $ip = self::$ARCHERFTP->get('host');
                ...
                if (is_file($myAdd)) {
                    ...
                    $remotefilescheck = self::$ARCHERFTP->listrecursive($remItem);
                    $opts = '-R';
                    $includeFile = '';
                    if (!$myAddItem) {
                        $myAddItem = $myAdd;
                ...
                $resp = self::$ARCHERURLRequests->isAvailable($testip, 1, 80);
                $avail = true;
                ...
                foreach ($filescheck as $j => &$filename) {
                    ...
                    if (!is_int($rindex)) {
                        ...
                        self::$ARCHERFTP->delete($remotefilename);
                    } else {
                        $localsize = self::getFilesize($localfilename);
                        $remotesize = null;
                        if ($avail) {
                            $rsize = self::$ARCHERURLRequests->process(
                                $sizeurl,
                                'POST',
                            ...
                            if (is_int($rsize)) {
                                ...
                                $sizeurl = sprintf('%s://%s/archer/status/getsize.php', 'https', $testip);
                                $rsize = self::$ARCHERURLRequests->process(
                                    $sizeurl,
                                    'POST',
                                    ['file' => base64_encode($remotefilename)]
                        ...
                        if (is_null($remotesize)) {
                            $remotesize = self::$ARCHERFTP->size($remotefilename);
                        }
                        if ($localsize == $remotesize) {
                            $localhash = self::getHash($localfilename);
                            $remotehash = null;
                            if ($avail) {
                                $rhash = self::$ARCHERURLRequests->process(
                                    $hashurl,
                                    'POST',
                                    ['file' => base64_encode($remotefilename)]
                                );
                                ...
                                if (strlen($rhash) == 64) {
                                    ...
                                    // we should re-try HTTPS because we don't know about the storage node setup
                                    // and letting curl follow the redirect doesn't work for POST requests
                                    $hashurl = sprintf('%s://%s/archer/status/gethash.php', 'https', $testip);
                                    $rhash = self::$ARCHERURLRequests->process(
                                        $hashurl,
                                        'POST',
                                        ['file' => base64_encode($remotefilename)]
