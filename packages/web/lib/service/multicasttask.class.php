 ...
 * @link     https://archer.umax.uz
 */
class MulticastTask extends ARCHERService
{
    /**
    ...
    public function __construct(
        ...
        parent::__construct();
        $overridePort = self::getSetting('ARCHER_MULTICAST_PORT_OVERRIDE');
        $this->_intID = $id;
        $this->_strName = $name;
        if ($overridePort) {
    ...
    public function getCMD()
    {
        ...
        list(
            ...
            array(
                'name' => array(
                    'ARCHER_MULTICAST_ADDRESS',
                    'ARCHER_MULTICAST_DUPLEX',
                    'ARCHER_MULTICAST_RENDEZVOUS',
                    'ARCHER_UDPCAST_MAXWAIT'
                )
            ),
            'value',
            false,
        ...
        if ($address) {
            $address = long2ip(
                ip2long($address) + (
                    ...
                    ) % self::getSetting('ARCHER_MULTICAST_MAX_SESSIONS')
                )
            );
        }
        $buildcmd = array(
