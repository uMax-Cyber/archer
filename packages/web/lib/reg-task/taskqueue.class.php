<?php
/**
 * The queue handling system for ARCHER's checkin/checkout processes.
 *
 * PHP version 5
 *
 * @category TaskQueue
...
class TaskQueue extends TaskingElement
{
    ...
    private function _email()
    {
        ...
        if (!$emailBinary) {
            $emailBinary = '/usr/sbin/sendmail -t -f noreply@archerserver.com -i';
        }
        $reg = '#\$\{server-name\}#';
        $nodeName = 'archerserver';
        ...
        if (!$fromEmail) {
            $fromEmail = 'noreply@archerserver.com';
        }
