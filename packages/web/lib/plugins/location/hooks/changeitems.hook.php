...
class ChangeItems extends Hook
{
        ...
        foreach ($LocationAssocs->locationassociations as &$LocationAssoc) {
            ...
            if ($StorageGroup->isValid()) {
                if (!isset($arguments['snapin'])
                    ...
                    && self::getSetting('ARCHER_SNAPIN_LOCATION_SEND_ENABLED') > 0)
                ) {
    ...
    public function bootItemSettings($arguments)
    {
        ...
        foreach ((array)self::getClass('LocationAssociationManager')
            ...
            $arguments['memdisk'] = "http://${ip}/archer/service/ipxe/$memdisk";
            $arguments['memtest'] = "http://${ip}/archer/service/ipxe/$memtest";
            $arguments['bzImage'] = "http://${ip}/archer/service/ipxe/$bzImage";
            $arguments['imagefile'] = "http://${ip}/archer/service/ipxe/$initrd";
            unset($Location);
        }
    }
    /**
    ...
    public function alterMasters($arguments)
    {
        ...
        if (!$arguments['ARCHERServiceClass'] instanceof MulticastManager) {
            return;
        }
    ...
    public function makeMaster($arguments)
    {
        ...
        if (!$arguments['ARCHERServiceClass'] instanceof MulticastTask) {
            return;
        }
