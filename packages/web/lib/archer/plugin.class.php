 ...
 * @link     https://archer.umax.uz
 */
class Plugin extends ARCHERController
{
    /**
    ...
    {
        $dir = trim(self::getSetting('ARCHER_PLUGINSYS_DIR'));
        if ($dir != '../lib/plugins/') {
            self::setSetting('ARCHER_PLUGINSYS_DIR', '../lib/plugins/');
            $dir = '../lib/plugins/';
        }
    ...
    public function getPlugins()
    {
        ...
        foreach ((array) $this->_getDirs() as &$file) {
            ...
            $Plugin
                ->set('name', $archer_plugin['name'])
                ->set('description', $archer_plugin['description']);
            $Plugin->_strPath = $file;
            $runFile = sprintf(
                '%s%s',
                $file,
                $archer_plugin['entrypoint']
            );
            $Plugin->_strEntryPoint = $runFile;
            $matchIcon = preg_match(
                '#^fa[-]?#',
                $archer_plugin['menuicon']
            );
            if ($matchIcon != false) {
