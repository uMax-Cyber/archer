<?php
/**
 * Boot menu for the archer pxe system
 *
 * PHP Version 5
 *
 * @category Bootmenu
 ...
 * @link     https://archer.umax.uz
 */
class BootMenu extends ARCHERBase
{
    /**
     * The kernel string
     *
    ...
    public function __construct()
    {
        ...
        $Send['booturl'] = array(
            '#!ipxe',
            "set archer-ip $webserver",
            sprintf('set archer-webroot %s', basename($curroot)),
            'set boot-url '
            . self::$httpproto
            . '://${archer-ip}/${archer-webroot}',
        );
        $this->_parseMe($Send);
        ...
        $global_field_test = 'ARCHER_BOOT_EXIT_TYPE';
        if ($_REQUEST['platform'] == 'efi') {
            $host_field_test = 'efiexit';
            $global_field_test = 'ARCHER_EFI_BOOT_EXIT_TYPE';
        }
        $StorageNodeID = @min(
            self::getSubObjectIDs(
                'StorageNode',
        ...
        $serviceNames = array(
            ...
            'ARCHER_KERNEL_ARGS',
            'ARCHER_KERNEL_DEBUG',
            'ARCHER_KERNEL_LOGLEVEL',
            'ARCHER_KERNEL_RAMDISK_SIZE',
            'ARCHER_KEYMAP',
            'ARCHER_KEY_SEQUENCE',
            'ARCHER_MEMTEST_KERNEL',
            'ARCHER_PXE_BOOT_IMAGE',
            'ARCHER_PXE_BOOT_IMAGE_32',
            'ARCHER_PXE_HIDDENMENU_TIMEOUT',
            'ARCHER_PXE_MENU_HIDDEN',
            'ARCHER_PXE_MENU_TIMEOUT',
            'ARCHER_TFTP_PXE_KERNEL',
            'ARCHER_TFTP_PXE_KERNEL_32',
        );
        list(
            $exit,
            $kernelArgs,
     ...
     * registered to the ARCHER Server.  This function allows actions
     * still occur
     *
    ...
    public function falseTasking($mc = false, $Image = false)
    {
        ...
        $serviceNames = array(
            ...
            'ARCHER_KERNEL_ARGS',
            'ARCHER_KERNEL_DEBUG',
            'ARCHER_MULTICAST_RENDEZVOUS',
            'ARCHER_NONREG_DEVICE'
        );
        list(
            $chkdsk,
            $kargs,
    ...
    public function printImageList()
    {
        ...
        if (!self::getSetting('ARCHER_IMAGE_LIST_MENU')) {
            if (!self::$Host->isValid()
    ...
    {
        if (self::$Host->isValid()
            && self::getSetting('ARCHER_NO_MENU')
        ) {
            $this->noMenu();
        }
        if ($this->_hiddenmenu) {
