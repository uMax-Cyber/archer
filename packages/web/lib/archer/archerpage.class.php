 ...
 * @link     https://archer.umax.uz
 */
abstract class ARCHERPage extends ARCHERBase
{
    /**
     * Name of the page
     *
        ...
        try {
            ...
            if (is_array($this->data) && count($this->data) < 1) {
                ...
                if ($this->data['error']) {
                    ...
                    $tablestr .= self::$archerlang['NoResults'];
                }
                $tablestr .= '</td></tr>';
                echo $tablestr;
                echo '</tbody>';
    ...
    public function deploy()
    {
        ...
        if ($this->obj instanceof Host) {
            ...
            echo '<option value="">- ';
            echo self::$archerlang['PleaseSelect'];
            echo ' -</option>';
        ...
        if ($TaskType->isInitNeededTasking()
            ...
            echo '<input type="checkbox" name='
                ...
                . (
                    self::getSetting('ARCHER_TASKING_ADV_SHUTDOWN_ENABLED') ?
                    ' checked' :
                    ''
                )
                . '/>';
        ...
        if ($TaskType->get('id') != 14) {
            ...
            echo '<input type="checkbox" name='
                ...
                . (
                    ...
                    '' :
                    (
                        self::getSetting('ARCHER_TASKING_ADV_WOL_ENABLED') ?
                        ' checked' :
                        ''
                    )
                )
        ...
        if (!$TaskType->isDebug()
            ...
            if ($TaskType->isInitNeededTasking()
                ...
                echo '<input type="checkbox" name='
                    ...
                    . (
                        self::getSetting('ARCHER_TASKING_ADV_DEBUG_ENABLED') ?
                        ' checked' :
                        ''
                    )
                    . '/>';
        ...
        if (!$TaskType->isDebug()
            ...
            echo ARCHERCron::buildSpecialCron('specialCrons');
            echo '</div>';
    ...
    public function deployPost()
    {
        ...
        try {
            ...
            case 'cron':
                ...
                if (!ARCHERCron::checkMinutesField($min)) {
                    throw new Exception(
                        sprintf(
                            '%s %s invalid',
                            'checkMinutesField',
                ...
                if (!ARCHERCron::checkHoursField($hour)) {
                    throw new Exception(
                        sprintf(
                            '%s %s invalid',
                            'checkHoursField',
                ...
                if (!ARCHERCron::checkDOMField($dom)) {
                    throw new Exception(
                        sprintf(
                            '%s %s invalid',
                            'checkDOMField',
                ...
                if (!ARCHERCron::checkMonthField($month)) {
                    throw new Exception(
                        sprintf(
                            '%s %s invalid',
                            'checkMonthField',
                ...
                if (!ARCHERCron::checkDOWField($dow)) {
                    throw new Exception(
                        sprintf(
                            '%s %s invalid',
                            'checkDOWField',
            ...
            try {
                ...
                if ($scheduleType == 'instant') {
                    ...
                    $ScheduledTask = self::getClass('ScheduledTask')
                        ...
                        ->set('other3', self::$ARCHERUser->get('name'))
                        ->set('isActive', 1)
                        ->set('other4', $wol);
    ...
    public function adFieldsToDisplay(
        ...
        $OUs = array_unique(
            array_filter(
                explode(
                    '|',
                    self::getSetting('ARCHER_AD_DEFAULT_OU')
                )
            )
        );
        $ADOU = trim($ADOU);
        ...
        try {
            ...
            if ($_SESSION['allow_ajax_kdl']
                ...
                if ($msg == 'dl') {
                    ...
                    self::$ARCHERURLRequests->process(
                        $_SESSION['dl-kernel-file'],
                        'GET',
                        false,
                        false,
                    ...
                    $orig = sprintf(
                        '/%s/%s',
                        trim(self::getSetting('ARCHER_TFTP_PXE_KERNEL_DIR'), '/'),
                        $destfile
                    );
                    $backuppath = sprintf(
                        '/%s/backup/',
                    ...
                    list(
                        ...
                        array(
                            'name' => array(
                                'ARCHER_TFTP_FTP_PASSWORD',
                                'ARCHER_TFTP_FTP_USERNAME',
                                'ARCHER_TFTP_HOST'
                            )
                        ),
                        'value',
                    ...
                    self::$ARCHERFTP
                        ...
                        ->connect();
                    if (!self::$ARCHERFTP->exists($backuppath)) {
                        self::$ARCHERFTP->mkdir($backuppath);
                    }
                    if (self::$ARCHERFTP->exists($orig)) {
                        self::$ARCHERFTP->rename($orig, $backupfile);
                    }
                    self::$ARCHERFTP
                        ->delete($orig)
                        ->rename($tmpfile, $orig)
    ...
    {
        $urls = array(
            'https://archer.umax.uz/globalusers',
            'https://archer.umax.uz/version/index.php?stable&dev'
        );
        $resp = self::$ARCHERURLRequests->process($urls);
    ...
    public function configure()
    {
        $Services = self::getSubObjectIDs(
            ...
            array(
                'name' => array(
                    'ARCHER_CLIENT_CHECKIN_TIME',
                    'ARCHER_CLIENT_MAXSIZE',
                    'ARCHER_GRACE_TIMEOUT',
    ...
    public function requestClientInfo()
    {
        if (isset($_POST['configure'])
            ...
            list(
                ...
                array(
                    'name' => array(
                        'ARCHER_CLIENT_BANNER_IMAGE',
                        'ARCHER_CLIENT_BANNER_SHA',
                        'ARCHER_CLIENT_CHECKIN_TIME',
                        'ARCHER_CLIENT_MAXSIZE',
                        'ARCHER_COMPANY_COLOR',
                        'ARCHER_COMPANY_NAME',
                        'ARCHER_GRACE_TIMEOUT',
                        'ARCHER_TASK_FORCE_REBOOT'
                    )
    ...
    public function import()
    {
        ...
        echo _('This page allows you to upload a CSV file into ARCHER to ease')
            . ' '
    ...
    public function newPMDisplay()
    {
        ...
        $fields = array(
            ...
            . ARCHERCron::buildSpecialCron('specialCrons')
            . '</div>'
            . '<div class="col-xs-12">'
