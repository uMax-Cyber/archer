<?php
/**
 * ARCHERController, individual SQL getters/setters.
 *
 * PHP Version 5
 *
 ...
 * @link     https://archer.umax.uz
 */
abstract class ARCHERController extends ARCHERBase
{
    /**
     * The data to set/get.
     *
                ...
                case 'createdBy':
                    if (!$val) {
                        if (self::$ARCHERUser->isValid()) {
                            $val = trim(self::$ARCHERUser->get('name'));
                        } else {
                            $val = 'archer';
                        }
                    }
    ...
    public function isValid()
    {
        try {
            foreach ($this->databaseFieldsRequired as &$key) {
                ...
                $val = $this->get($key);
                if (!is_numeric($val) && !$val) {
                    throw new Exception(self::$archerlang['RequiredDB']);
                }
                unset($key);
            }
            if ($this->get('id') < 1) {
