 ...
 * @link     https://archer.umax.uz
 */
class User extends ARCHERController
{
    /**
    ...
    public function validatePw(
        ...
        if ($this->passwordValidate($username, $password)) {
            ...
            if (session_status() != PHP_SESSION_NONE) {
                $_SESSION['ARCHER_USER'] = $this->get('id');
            }
            self::log(
                sprintf(
                    '%s %s.',
            ...
            if (self::$ARCHERUser->isValid()) {
                ...
                $this
                    ->set('id', self::$ARCHERUser->get('id'))
                    ->set('name', self::$ARCHERUser->get('name'))
                    ->set('password', '', true)
                    ->set('type', $type);
                if (!$this->_sessionID) {
                    $this->_sessionID = session_id();
    ...
    private function _isLoggedIn()
    {
            ...
            list(
                ...
                array(
                    'name' => array(
                        ...
                        'ARCHER_INACTIVITY_TIMEOUT',
                        'ARCHER_REGENERATE_TIMEOUT',
                    )
                ),
                'value',
                false,
        ...
        if (session_status() != PHP_SESSION_NONE) {
            ...
            if (!$this->get('authIP')
                ...
                if ($active >= $timeout) {
                    self::setMessage(self::$archerlang['SessionTimeout']);
                    if (isset($_SESSION['OBSOLETE'])) {
                        $_SESSION['OBSOLETE'] = true;
                    }
                }
    ...
    public function logout()
    {
        ...
        $messages = $_SESSION['ARCHER_MESSAGES'];
        // Destroy session
