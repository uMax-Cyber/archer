 ...
 * @link     https://archer.umax.uz
 */
class ReportMaker extends ARCHERBase
{
    /**
    ...
    public function outputReport($intType = 0, $nojson = false)
    {
        ...
        if (isset($_POST['nojson']) || true === $nojson) {
            ...
            case 2:
                ...
                $logoimage = trim(
                    self::getSetting('ARCHER_CLIENT_BANNER_IMAGE')
                );
                if ($logoimage) {
                    $logoimage = sprintf(
                        '--logoimage %s',
                        escapeshellarg(
                            sprintf(
                                'http%s://%s/archer/management/other/%s',
                                (
