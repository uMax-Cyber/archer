<?php
/**
 * The host object (main item ARCHER deals with
 *
 * PHP version 5
 *
 * @category Host
 ...
 * @link     https://archer.umax.uz
 */
class Host extends ARCHERController
{
    /**
     * The host table
     *
    ...
    public function destroy($key = 'id')
    {
        ...
        self::getClass('ModuleAssociationManager')
            ->destroy($find);
        self::getClass('GreenArcherManager')
            ->destroy($find);
    ...
    private function _setAlo()
    {
        ...
        if (!$this->get('hostalo')->isValid()) {
            self::$_hostalo = self::getSetting('ARCHER_CLIENT_AUTOLOGOFF_MIN');
        } else {
    ...
    public function createImagePackage(
        ...
        try {
            if (!$this->isValid()) {
                throw new Exception(self::$archerlang['HostNotValid']);
            }
            $Task = $this->get('task');
            $TaskType = new TaskType($taskTypeID);
            if (!$TaskType->isValid()) {
                throw new Exception(self::$archerlang['TaskTypeNotValid']);
            }
            if ($Task->isValid()) {
                $iTaskType = $Task->getTaskType()->isImagingTask();
                if ($iTaskType) {
                    throw new Exception(self::$archerlang['InTask']);
            ...
            if ($imagingTypes) {
                if (!$Image->isValid()) {
                    throw new Exception(self::$archerlang['ImageNotValid']);
                }
                ...
                if (!$StorageGroup->isValid()) {
                    throw new Exception(self::$archerlang['ImageGroupNotValid']);
                }
            ...
            $username = ($username ? $username : self::$ARCHERUser->get('name'));
            if (!$Task->isValid()) {
                ...
                if (!$Task->save()) {
                    throw new Exception(self::$archerlang['FailedTask']);
                }
                $this->set('task', $Task);
            }
            ...
            if ($TaskType->isMulticast()) {
                ...
                if ($MulticastSession instanceof MulticastSession
                    ...
                    $port = self::getSetting('ARCHER_UDPCAST_STARTINGPORT');
                    $portOverride = self::getSetting('ARCHER_MULTICAST_PORT_OVERRIDE');
                    ...
                    if ($MulticastSession->save()) {
                        $assoc = true;
                        if (!self::getSetting('ARCHER_MULTICAST_PORT_OVERRIDE')) {
    ...
    {
        $limit = self::getSetting('ARCHER_SNAPIN_LIMIT');
        if ($limit > 0) {
