 ...
 * @link     https://archer.umax.uz
 */
class StorageNode extends ARCHERController
{
    /**
    ...
    {
        $test = self::$ARCHERURLRequests->isAvailable($this->get('ip'), 1);
    ...
    private function _getData($item)
    {
        ...
        $url = sprintf(
            '%s://%s/archer/status/getfiles.php?path=%s',
            self::$httpproto,
            $this->get('ip'),
            $items[$item]
        );
        $response = self::$ARCHERURLRequests->process($url);
        return preg_grep(
    ...
    {
        $used = explode(',', self::getSetting('ARCHER_USED_TASKS'));
        if (count($used) < 1) {
            $used = array(
                1,
                15,
