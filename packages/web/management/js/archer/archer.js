/**
 * ARCHER Main Javascript File.
 *
 * category: ARCHERProject
 ...
 */
$.fn.archerAjaxSearch = function(opts) {
    if (this.length == 0) {
        return this;
    }
    var Defaults = {
    ...
    return this.each(function(evt) {
        ...
        function PerformSearch() {
            ...
            this.SearchAJAX = $.ajax({
                ...
                data: {crit: Query},
                beforeSend: function() {
                    SubmitButton.addClass('searching').find('i').removeClass().addClass('archersearch fa fa-spinner fa-pulse fa-fw');
                },
                success: function(response) {
                    dataLength = response === null || response.data === null ? dataLength = 0 : response.data.length;
                    SubmitButton.removeClass('searching').find('i').removeClass().addClass('archersearch fa fa-search');
...
/**
 * Handles archervariable elements.
 */
$.fn.archerVariable = function(opts) {
...
(function($) {
    /**
     * ARCHER Settings quick image os display.
     */
    $('#ARCHER_QUICKREG_IMG_ID').on('change',function() {
        $.ajax({
            ...
            success: function(idata) {
                $('#ARCHER_QUICKREG_OS_ID').html(idata.replace(/\"/g,""));
            }
        });
    });
     ...
     */
    $('.search-input').archerAjaxSearch();
    /**
     * The ARCHER variable definer.
     */
    $('.archer-variable').archerVariable();
    /**
    ...
    }).on('click', '.archerpasswordeye', function(e) {
        e.preventDefault();
    ...
    $(':password')
        ...
        .before('<span class="input-group-addon"><i class="fa fa-eye-slash archerpasswordeye"></i></span>');
    /**
...
function setEditFocus() {
    $('input, textarea').not('[type="number"], [type="checkbox"], [name="groupsel"], [name="nodesel"], [name="ulang"], #uname, #upass, .system-search, .search-input, [type="radio"], [readonly], .tablesorter-filter').on('focus', function(e) {
        ...
        field = $(this);
        $(this).after(
            '<span class="input-group-addon archerpencil">'
            + '<i class="fa fa-pencil fa-fw archerpencil"></i>'
            + '</span>'
        );
    }).blur(function(e) {
        e.preventDefault();
...
function loginDialog(
    ...
    exportauth = $('.archer-export').val();
    deleteauth = $('.archer-delete').val();
    authneeded = true;
    label = submitButton;
    close = closeButton;
    title = titleText;
    ...
    if (authneeded) {
        BootstrapDialog.show({
            ...
            message: 'Enter GUI Information<br/>'
                ...
                + '<input type="text" name="archerguiuser" id="username" class="form-control" required/>'
                + '</div>'
                + '</div>'
                + '<div class="form-group">'
                + '<label for="password">'
                ...
                + '</label>'
                + '<div class="input-group">'
                + '<input type="password" name="archerguipass" id="password" class="form-control" required/>'
                + '</div>'
                + '</div>',
            buttons: [{
                label: label,
                ...
                action: function(dialogItself) {
                    username = $('[name="archerguiuser"]').val();
                    password = $('[name="archerguipass"]').val();
                    ajaxRun(
                        username,
                        password,
                        url,
...
function ajaxRun(
    ...
    if (authneeded) {
        $.extend(
            gdata,
            {
                archerguiuser: username,
                archerguipass: password
            }
        );
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.error) {
                ...
                dialog.close();
                if (authneeded) {
                    $('<form id="'+formid+'" method="post" action="'+url+'"><input type="hidden" name="archerguiuser" value="'+username+'"/><input type="hidden" name="archerguipass" value="'+password+'"/><input type="hidden" name="nojson"/></form>').appendTo('body').submit().remove();
                } else {
 ...
 */
function setupArcherTableInformation() {
    if (typeof $.tablesorter == 'undefined') {
        return;
    }
